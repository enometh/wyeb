                          ...
                          gpointer      user_data)
{
  ...
  g_string_append_printf (html, "<html><style>%s</style>"
                          "<title>%s</title>"
...
readability_mode(Win *win)
{
  ...
  webkit_web_view_evaluate_javascript (win->kit, JS_READABILITY, -1, NULL, NULL, NULL, readability_js_finish_cb, win);
  if (!orig)
